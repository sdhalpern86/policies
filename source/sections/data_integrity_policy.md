...
1healthy.world takes data integrity very seriously. As stewards and partners of 1healthy.world Customers, we strive to assure data is protected from unauthorized access and that it is available when needed. The following policies drive many of our procedures and technical settings in support of the 1healthy.world mission of data protection.
...
* All access to Production Systems must be logged. This is done following the 1healthy.world Auditing Policy.

### Prevention of malware on Production Systems

* All Production Systems must have Microsoft Azure tools running at set to scan system every 2 hours and at reboot to assure not malware is present. Detected malware is evaluated and removed.
* All Production Systems are to only be used for 1healthy.world business needs.

### Patch Management

* Patches, application, and system OS versions are kept up to date at all times. New versions are tested.
* Administrators subscribe to mailing lists to assure up to date on current version of all 1healthy.world managed software on Production Systems.
...
* Ensure 1healthy.world customer Production Data is segmented and only accessible to customer authorized to access data.
* All Production Data at rest is stored on encrypted volumes. Encryption at rest is ensured through the use of automated deployment scripts referenced in the 1healthy.world Configuration Management Policy.
...
* In the case of 1healthy.world provided APIs, provide mechanisms to assure person sending or receiving data is authorized to send and save data.
